...
export default function Faq1MediaTab() {
    ...
    useEffect(() => {
        ...
        async function load() {
            try {
                const snap = await getDoc(doc(db, "faq1_media", "main"));
                const data = snap.exists() ? (snap.data() as any) : null;
                if (!mounted) return;
                setMedia(data?.MEDIA ?? DEFAULT_CONTENT.FAQ1_MEDIA);
            } catch (err) {
    ...
    async function uploadFile(file: File) {
        ...
        try {
            ...
            let url = "";
            if (type === "image") url = await uploadImageFile(file, "faq1");
            else url = await uploadVideoFile(file, "faq1");

            const newMedia = { ...media, type, src: url, alt: file.name };
            setMedia(newMedia);
            await setDoc(doc(db, "faq1_media", "main"), { MEDIA: newMedia });
            toast.dismiss(t);
    ...
    async function save() {
        try {
            await setDoc(doc(db, "faq1_media", "main"), { MEDIA: media });
            toast.success("Media FAQ1 guardada");
        } catch (e: any) {
    ...
    if (loading) return <div className="p-4">Cargando media FAQ1...</div>;

    return (
        <section className="mb-6 p-4 border rounded bg-background/50">
            <h2 className="font-semibold mb-3">Media — FAQ 1</h2>
